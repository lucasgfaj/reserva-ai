  ...
  describe('validate', () => {
    ...
    describe('condominiumName validation', () => {
      ...
      it('should reject name shorter than 3 characters', () => {
        ...
        const result = validator.validate(input);
        expect(result.isValid).toBe(false);
        expect(result.errors).toContain('Nome do condomínio deve ter pelo menos 3 caracteres');
      });
      ...
      it('should reject name longer than 150 characters', () => {
        ...
        const result = validator.validate(input);
        expect(result.isValid).toBe(false);
        expect(result.errors).toContain('Nome do condomínio não pode exceder 150 caracteres');
      });
    });
    ...
    describe('condominiumAddress validation', () => {
      ...
      it('should reject address longer than 255 characters', () => {
        ...
        const result = validator.validate(input);
        expect(result.isValid).toBe(false);
        expect(result.errors).toContain('Endereço não pode exceder 255 caracteres');
      });
    });
    ...
    describe('adminEmail validation', () => {
      it('should reject empty email', () => {
        ...
        const result = validator.validate(input);
        expect(result.isValid).toBe(false);
        expect(result.errors).toContain('E-mail do administrador é obrigatório');
      });
      ...
      it('should reject invalid email format', () => {
        ...
        const result = validator.validate(input);
        expect(result.isValid).toBe(false);
        expect(result.errors).toContain('E-mail do administrador deve ser válido');
      });
    });
    ...
    describe('adminPassword validation', () => {
      ...
      it('should reject password without letter', () => {
        ...
        const result = validator.validate(input);
        expect(result.isValid).toBe(false);
        expect(result.errors).toContain('Senha deve conter pelo menos uma letra, um número e um caractere especial (!@#$%^&*)');
      });
